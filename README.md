# TryCatchWrapper
源码在src/index.js，需要转换的js代码在src/code.js
...
---|---
BinaryExpression | 二元运算，比如+-*/
ConditionExpression | 三元运算()? : 
...
当然，这里面所列的不是全的，只是一些大家听过比较多，运用应该也是比较都的。如果要了解更多，可以通过babel-types的官网去了解更多的。[点击进入](https://babeljs.io/docs/en/next/babel-types.html)

### 抽象语法树的运用
简单来说只要用到语法分析或者修改修改源码的，都可以利用抽象语法树来完成，比如typescript语法校验、IDE的语法校验高亮、ulglify-js、以及babel的tramsform、@babel/preset-react等等。可想而知，抽象语法树运用还是很广的。
...
- babel-types，babel工具包，可以用来判断一个树节点是什么类型以及创建statement、declaration以及declaration树节点。  
...
### 定义访问器
在上面对AST的介绍中，我们可以找到函数可能出现的节点有三个，FunctionExpression、ArrowFuntionExpression和FunctionDeclaration，因此我们就可以定义我们的访问器了。
...
### 生成catch体
catch是一个类型函数的东西，但它又不是，它接受一个参数，这个参数就是Error对象，catch体的内容我们一般会分析这个error，然后上报到后端，但是很多时候我们的代码是经过混淆压缩的，所以error里面我们不能获取到具体发生错误的具体行列以及发生error的函数名。因此我们在访问函数节点的时候，需要事先获取这些信息，并把它塞到原来的catch体里面。**这里的行列只能是函数开始的行列，不能定位到error发生的行列。**

获取行列信息很简单，通过获取节点的loc属性便可。但是获取函数名，就比较复杂了，函数的定义，有三种方式，如果不是以函数申明的方式定义函数，那么函数名的获取，需要从它父节点中获取，并且还要判断父节点的类型，如果是普通的函数表达式，从父节点的id属性获取，如果是对象属性函数，那么需要从父节点的key属性获取。实际上还有类的情况没考虑，这里先不考虑，后续有时间再补充。还有一种情况就是函数可能是匿名的。综合考虑后，获取函数名的代码如下：
```
let visitor = {
  'ArrowFunctionExpression|FunctionExpression|FunctionDeclaration': function(path) {
      let node = path.node
...
```
有行列以及函数名，我们需要生成一个变量申明的AST节点，点击[这里](https://babeljs.io/docs/en/next/babel-types.html#variabledeclaration)获取如何生成申明变量的节点，代码如下：
``` js
...
```
接着我们需要知道如何把处理异常的handler函数的函数体转为catch里面的内容，这个时候就需要用到@babel/template包了，这里直接贴代码，怎么使用@babel/template，点击[这里](https://babeljs.io/docs/en/babel-template#templatestatement)。
```
...
```
let catchClause = types.catchClause(types.identifier('error'), types.blockStatement(
    [infoDeclarator, catchBody]
));
let tryStatement = types.tryStatement(body, catchClause);
```
### 替换原来的函数内容
...
if (types.isFunctionDeclaration(node)) {
    func = types.functionDeclaration(node.id, params, types.BlockStatement([tryStatement]), node.enerator, node.async);
} else {
    func = types.functionExpression(node.id, params, types.BlockStatement([tryStatement]), node.enerator, node.sync);
}
